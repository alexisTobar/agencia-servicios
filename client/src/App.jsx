...
export default function App() {
  ...
  // --- FUNCIÓN DE ENVÍO DIRECTO (CORREO DESDE BACKEND) ---
  const enviarConsultaBackend = async (e) => {
    e.preventDefault();
    const loading = toast.loading("Enviando consulta...");
    try {
      await axios.post(`${API_URL}/contacto`, form);
      toast.success("¡Consulta enviada! Te responderemos al correo.", { id: loading });
      setForm({ nombre: '', email: '', mensaje: '' });
    } catch (err) {
      toast.error("Error al enviar. Intenta de nuevo.", { id: loading });
    }
  };
  ...
  return (
    <div className={`${darkMode ? 'dark' : ''} text-left`}>
      <div className="min-h-screen text-slate-900 dark:text-white transition-colors duration-500 antialiased relative text-left">
        ...
        <header id="inicio" className="pt-32 md:pt-44 pb-20 md:pb-32 px-4 md:px-6 max-w-7xl mx-auto overflow-visible">
          <div className="grid lg:grid-cols-2 gap-8 md:gap-12 items-center text-left">
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="z-10 text-left">
              ...
              {/* TÍTULO: PALABRAS FIJAS IGUALES, DINÁMICAS GIGANTES */}
              ...
              <div className="flex flex-col sm:flex-row gap-4 text-left text-left">
                ...
                <a href="#web" className="text-center bg-white/10 border border-white/10 px-8 py-4 rounded-2xl md:rounded-3xl font-black uppercase text-xs transition-all hover:bg-white/20 text-left text-left text-left">Ver Planes</a>
              </div>
            </motion.div>
            ...
            <div className="relative mt-8 lg:mt-0 flex justify-center lg:justify-end text-left">
              ...
              <motion.div 
                ...
                className="w-full max-w-[500px] lg:max-w-[120%] flex justify-center lg:justify-end text-left"
              >
                <img 
                  src="https://i.postimg.cc/DwGDHxxH/Copilot-20260115-120913.png" 
                  alt="Empreweb Presentación" 
                  className="w-full h-auto object-contain drop-shadow-[0_35px_35px_rgba(0,0,0,0.4)] scale-110 md:scale-125 lg:scale-150 text-left" 
                />
              </motion.div>
            </div>
          </div>
        ...
        <section id="servicios-principales" className="py-24 px-6 max-w-7xl mx-auto text-left text-left">
          <div className="text-center mb-20 md:mb-24 text-left text-left">
            <h3 className="text-3xl md:text-5xl font-black tracking-tighter uppercase italic text-left">Mis Servicios <span className="text-rose-500 underline decoration-indigo-500 underline-offset-8 text-left text-left">Principales</span></h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 text-center text-left text-left">
            {items.filter(i => i.categoria === 'principal').map((item) => (
              <div key={item._id} className="relative group flex flex-col items-center text-left text-left">
                ...
                {isAdminMode ? (
                  <div className="space-y-2 w-full text-left text-left">
                    ...
                    <button onClick={() => guardarCambios(item._id)} className="bg-green-600 text-white px-4 py-1 rounded text-[10px] font-bold uppercase text-left text-left text-left">SAVE</button>
                  </div>
                ) : (
                  <>
                    <h4 className="text-xl font-black uppercase italic mb-4 text-left text-left">{item.titulo}</h4>
                    <p className="text-slate-500 dark:text-slate-400 text-sm px-4 leading-relaxed font-medium text-left text-left text-left text-left">{item.desc}</p>
                  </>
                )}
              </div>
            ))}
            {isAdminMode && <button onClick={() => addItem('principal')} className="border-4 border-dashed rounded-[3rem] p-12 text-slate-300 hover:text-rose-500 transition-all text-left text-left text-left"><Plus size={48}/></button>}
          </div>
        </section>

        {/* --- SECCIÓN WEB CORPORATIVA --- */}
        <section id="web" className="py-20 md:py-32 px-4 md:px-6 max-w-7xl mx-auto text-left text-left text-left text-left text-left">
          <div className="flex justify-between items-end mb-12 border-b border-indigo-500/20 pb-4 text-left text-left text-left text-left">
            <div>
              <h3 className="text-3xl md:text-5xl font-black tracking-tighter uppercase italic text-left text-left text-left">Webs <span className="text-indigo-500 text-left text-left text-left text-left">Corporativas</span></h3>
              <p className="text-slate-400 font-bold uppercase text-[8px] md:text-[10px] tracking-widest mt-2 text-left text-left text-left">Plataformas de alta gama</p>
            </div>
            {isAdminMode && <button onClick={() => addItem('web')} className="bg-indigo-500 p-2 rounded-full text-white shadow-xl text-left text-left text-left text-left text-left"><Plus size={20}/></button>}
          </div>
        ...
        <section id="landing" className="py-20 md:py-32 px-4 md:px-6 max-w-7xl mx-auto bg-slate-900/5 dark:bg-white/5 rounded-[3rem] md:rounded-[5rem] mb-20 md:mb-32 text-left text-left text-left text-left">
          <div className="flex justify-between items-end mb-12 border-b border-rose-500/20 pb-4 px-4 text-left text-left text-left text-left">
            <div>
              <h3 className="text-3xl md:text-5xl font-black tracking-tighter uppercase italic text-rose-500 text-left text-left text-left">Landing Pages</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 px-4 text-left text-left text-left text-left">
        ...
        {/* --- SECCIÓN CONTACTO (VÍA CORREO SILENCIOSO) --- */}
        <section id="contacto" className="py-20 md:py-32 px-4 md:px-6 text-left text-left text-left">
          <div className="max-w-5xl mx-auto grid lg:grid-cols-2 bg-white dark:bg-white/5 rounded-[3rem] md:rounded-[5rem] border border-white/10 shadow-2xl backdrop-blur-xl overflow-hidden text-left text-left text-left text-left">
            <div className="p-8 md:p-16 lg:p-20 bg-indigo-600 text-white flex flex-col justify-center text-left text-left text-left">
              <h3 className="text-4xl md:text-5xl font-black tracking-tighter mb-8 italic uppercase leading-none text-left text-left">Hablemos hoy.</h3>
              <p className="text-indigo-100 mb-10 text-base md:text-lg font-medium text-left text-left">Inicia tu proyecto enviándonos una consulta directa. Recibirás una respuesta en tu correo.</p>
              <div className="space-y-5 font-bold text-xs md:text-sm tracking-widest text-left text-left text-left">
                <div className="flex gap-4 items-center text-left text-left text-left"><Mail size={22}/> contacto@empreweb.com</div>
                <div className="flex gap-4 items-center text-left text-left text-left text-left text-left"><MapPin size={22}/> Talagante, Chile</div>
              </div>
            </div>
            <div className="p-8 md:p-16 lg:p-20 text-left text-left text-left text-left">
              <form className="grid gap-6 text-left text-left" onSubmit={enviarConsultaBackend}>
                <input className="bg-slate-100 dark:bg-black/40 p-6 rounded-3xl outline-none font-bold text-xs text-slate-900 dark:text-white border border-transparent focus:border-indigo-500 transition-all text-left text-left" placeholder="NOMBRE COMPLETO" required value={form.nombre} onChange={e => setForm({...form, nombre: e.target.value})} />
                <input className="bg-slate-100 dark:bg-black/40 p-6 rounded-3xl outline-none font-bold text-xs text-slate-900 dark:text-white border border-transparent focus:border-indigo-500 transition-all text-left text-left" placeholder="EMAIL" required type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})} />
                <textarea className="bg-slate-100 dark:bg-black/40 p-6 rounded-3xl outline-none font-bold text-xs h-40 resize-none text-slate-900 dark:text-white border border-transparent focus:border-indigo-500 transition-all text-left text-left" placeholder="DETALLES DEL PROYECTO" required value={form.mensaje} onChange={e => setForm({...form, mensaje: e.target.value})} />
                <button type="submit" className="bg-indigo-600 py-6 rounded-3xl font-black text-white uppercase text-[10px] tracking-widest shadow-xl transition-all hover:bg-rose-600 text-left text-center">Solicitar Propuesta de Proyecto <Send size={14} className="inline ml-2"/></button>
              </form>
            </div>
          </div>
        </section>

        <footer className="py-20 text-center opacity-30 text-[8px] font-black uppercase tracking-[1.5em] text-left text-center text-left text-left text-left">© 2026 EMPREWEB CHILE</footer>
        {!isLogged && <div onClick={() => setShowLogin(true)} className="fixed bottom-0 left-0 w-24 h-24 cursor-default z-[100] opacity-0 text-left text-left text-left text-left" title="Admin Login" />}
        
        <AnimatePresence>
          {showLogin && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[120] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 text-left text-left text-left">
              <div className="bg-white dark:bg-[#0a0a0a] p-10 rounded-[3rem] w-full max-w-sm text-center border border-white/10 shadow-2xl text-left text-left">
                <Terminal size={40} className="mx-auto mb-6 text-indigo-500 text-left text-left" />
                <h4 className="text-xl md:text-2xl font-black mb-6 italic uppercase text-indigo-500 text-center text-left text-left">Acceso Maestro</h4>
                ...
                <button onClick={handleLogin} className="w-full bg-indigo-600 py-3 md:py-4 rounded-xl md:rounded-2xl font-black text-white uppercase tracking-widest text-xs transition-all hover:bg-indigo-700 text-left text-center text-left text-left text-left">Desbloquear</button>
...
const PriceCard = ({ item, isAdmin, onUpdate, onDelete, color = "indigo" }) => {
  ...
  return (
    <motion.div whileHover={{ y: -8 }} className={`bg-white/40 dark:bg-white/5 backdrop-blur-lg border border-slate-200 dark:border-white/10 p-8 md:p-12 rounded-[3.5rem] md:rounded-[4.5rem] flex flex-col h-full border-b-[12px] md:border-b-[16px] ${currentTheme} transition-all shadow-2xl text-left text-left text-left`}>
      {isAdmin ? (
        <div className="space-y-4 text-left text-left text-left text-left text-left">
          ...
          <div className="flex gap-2 text-left text-left text-left text-left text-left">
            <button onClick={() => onUpdate(item._id, edit)} className="flex-1 bg-green-600 text-white py-3 rounded-2xl text-[10px] font-bold uppercase transition-all hover:bg-green-700 text-left text-center text-left text-left text-left">Guardar</button>
            <button onClick={onDelete} className="bg-rose-600 text-white px-4 rounded-2xl transition-all hover:bg-rose-700 text-left text-left text-left text-left text-left"><Trash2 size={16}/></button>
          </div>
        </div>
      ) : (
        <>
          <h4 className="text-3xl md:text-4xl font-black tracking-tighter mb-4 leading-none text-left text-left text-left text-left">{item.titulo}</h4>
          <div className={`text-2xl md:text-3xl font-black mb-10 italic text-left text-left text-left text-left`}>{item.precio}</div>
          <ul className="space-y-4 md:space-y-5 mb-10 md:mb-14 flex-grow text-left text-left text-left text-left text-left">
