...
export default function App() {
  ...
  const enviarConsultaBackend = async (e) => {
    ...
    try {
      ...
      toast.success("¡Consulta enviada! Te responderemos al correo.", { id: loadingToast });
  ...
  return (
    <div className={`${darkMode ? 'dark' : ''} text-left`}>
      <div className="min-h-screen text-slate-900 dark:text-white transition-colors duration-500 antialiased relative">
        ...
        {/* --- SECCIÓN ADICIONALES --- */}
        <section id="adicional" className="py-20 md:py-32 px-4 md:px-6 max-w-7xl mx-auto text-left text-left">
        ...
        <section id="resenas" className="py-32 px-6 max-w-7xl mx-auto text-left text-left">
          ...
          <Marquee speed={35} pauseOnHover gradient={false} className="text-left">
            {resenas.map((r) => (
            ...
            {showResenaForm && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-md flex items-center justify-center p-4 text-left">
                <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white dark:bg-[#0a0a0a] p-10 rounded-[3rem] w-full max-w-sm text-center border border-white/10 shadow-2xl text-left">
                  ...
                  <h4 className="text-2xl font-black mb-6 italic uppercase text-indigo-500 text-center text-left">Nueva Reseña</h4>
                  <form onSubmit={submitResena} className="space-y-4 text-left">
                    <input className="bg-slate-100 dark:bg-white/5 w-full p-4 rounded-2xl outline-none font-bold text-sm text-slate-900 dark:text-white border border-transparent focus:border-indigo-500 transition-all text-left" placeholder="TU NOMBRE" required value={nuevaResena.nombre} onChange={e => setNuevaResena({ ...nuevaResena, nombre: e.target.value })} />
                    <textarea className="bg-slate-100 dark:bg-white/5 w-full p-4 rounded-2xl outline-none font-bold text-sm h-32 resize-none text-slate-900 dark:text-white border border-transparent focus:border-indigo-500 transition-all text-left" placeholder="COMENTARIO" required value={nuevaResena.comentario} onChange={e => setNuevaResena({ ...nuevaResena, comentario: e.target.value })} />
                    ...
                    <button type="submit" className="w-full bg-indigo-600 py-4 rounded-xl font-black text-white uppercase tracking-widest text-xs transition-all hover:bg-indigo-700 text-left text-center">Publicar</button>
                  </form>
                </motion.div>
              </motion.div>
            )}
        ...
        <section id="contacto" className="py-20 md:py-40 px-6 max-w-5xl mx-auto z-10 relative">
          ...
          <form onSubmit={enviarConsultaBackend} className="grid gap-6 bg-white dark:bg-zinc-900/50 p-8 md:p-12 rounded-[3rem] border border-zinc-200 dark:border-white/10 shadow-2xl text-left">
            ...
            <button 
              disabled={enviando} 
              className="w-full bg-indigo-600 text-white py-6 rounded-3xl font-black uppercase text-[12px] tracking-[0.2em] italic hover:bg-indigo-700 transition-all flex items-center justify-center gap-3 shadow-xl text-left text-center"
            >
        ...
        <footer className="py-20 text-center opacity-30 text-[8px] font-black uppercase tracking-[1.5em] text-left text-center">© 2026 EMPREWEB CHILE</footer>
          ...
          {showLogin && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[120] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 text-left text-left text-left">
              <div className="bg-white dark:bg-[#0a0a0a] p-10 rounded-[3rem] w-full max-w-sm text-center border border-white/10 shadow-2xl text-left text-left">
                ...
                <button onClick={handleLogin} className="w-full bg-indigo-600 py-3 md:py-4 rounded-xl md:rounded-2xl font-black text-white uppercase tracking-widest text-xs transition-all hover:bg-indigo-700 text-left text-center text-left text-left text-left text-left">Desbloquear</button>
...
const PriceCard = ({ item, isAdmin, onUpdate, onDelete, color = "indigo" }) => {
  ...
  const themeColors = { rose: "border-rose-500 text-rose-500", indigo: "border-indigo-500 text-indigo-500", orange: "border-orange-500 text-orange-500 text-left" };
    ...
    <motion.div whileHover={{ y: -8 }} className={`bg-white/40 dark:bg-white/5 backdrop-blur-lg border border-slate-200 dark:border-white/10 p-8 md:p-12 rounded-[3.5rem] md:rounded-[4.5rem] flex flex-col h-full border-b-[12px] md:border-b-[16px] ${currentTheme} transition-all shadow-2xl text-left text-left text-left`}>
      {isAdmin ? (
        <div className="space-y-4 text-left text-left">
          <input className="w-full bg-black/20 p-3 rounded-2xl text-xs text-white outline-none border border-white/5 focus:border-indigo-500 text-left text-left" value={edit.titulo} onChange={e => setEdit({ ...edit, titulo: e.target.value })} />
          <input className="w-full bg-black/20 p-3 rounded-2xl text-xs font-black text-indigo-500 outline-none border border-white/5 focus:border-indigo-500 text-left text-left" value={edit.precio} onChange={e => setEdit({ ...edit, precio: e.target.value })} />
          <textarea className="w-full bg-black/20 p-3 rounded-2xl text-xs h-24 text-white outline-none border border-white/5 focus:border-indigo-500 text-left text-left" value={edit.desc} onChange={e => setEdit({ ...edit, desc: e.target.value })} />
        ...
        <>
          ...
          <div className={`text-2xl md:text-3xl font-black mb-10 italic text-left text-left text-left text-left text-left`}>{item.precio}</div>
