...
import {
  ...
  Layers, Globe, Palette, Megaphone, Target, User, Laptop, Terminal, ChevronDown, Instagram, Github
} from 'lucide-react';
...
export default function App() {
  ...
  const faqs = [
    ...
    { q: "¿Costo por productos adicionales?", a: "La subida inicial es gratis. Si deseas que yo suba más, el costo es de 5 USD en plan E-commerce y 3 USD en Premium." },
    { q: "¿Beneficios de optimización?", a: "Realizamos un proceso manual y optimizado para SEO, mejorando atributos y descripciones para maximizar la visibilidad en buscadores." },
    ...
    { q: "¿Tendrá mantenimiento?", a: "Incluido gratis los primeros días según el plan. Luego es opcional con costo mensual para mantener todo al 100%." }
  ];
      ...
      <div className="min-h-screen text-slate-900 dark:text-white transition-colors duration-500 antialiased relative">
        ...
        <header id="inicio" className="pt-44 pb-32 px-6 max-w-7xl mx-auto grid lg:grid-cols-2 items-center gap-12 text-left">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="z-10">
            ...
            <h2 className="text-5xl md:text-7xl font-black tracking-tighter leading-[1.1] mb-8">
              ...
              <div className="inline-block relative">
                <AnimatePresence mode="wait">
                  <motion.span key={palabras[indexPalabra]} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="block text-7xl md:text-9xl text-indigo-500 font-black italic leading-none py-4">
            ...
            <div className="flex gap-4">
              <a href="#contacto" className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black uppercase text-xs shadow-xl transition-all hover:bg-indigo-700">Comenzar ahora</a>
            </div>
          </motion.div>
        ...
        <section id="adicional" className="py-20 px-4 md:px-6 max-w-7xl mx-auto text-left">
          <div className="flex justify-between items-end mb-12 border-b border-orange-500/20 pb-8 text-left text-left">
            <h3 className="text-3xl md:text-5xl font-black tracking-tighter uppercase italic text-orange-500">Servicios <span className="dark:text-white text-slate-900 font-black text-left text-left">Adicionales</span></h3>
            {isAdminMode && <button onClick={() => addItem('adicional')} className="bg-orange-500 p-2 rounded-full text-white shadow-xl"><Plus size={20}/></button>}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <section id="resenas" className="py-24 px-6 max-w-7xl mx-auto text-left overflow-hidden">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            ...
            <div>
              ...
              <form onSubmit={enviarConsultaBackend} className="grid gap-6 bg-white dark:bg-zinc-900/50 p-8 md:p-12 rounded-[3rem] border border-zinc-200 dark:border-white/10 shadow-2xl">
                ...
                <button disabled={enviando} className="w-full bg-indigo-600 text-white py-6 rounded-3xl font-black uppercase text-[12px] tracking-[0.2em] italic hover:bg-indigo-700 transition-all flex items-center justify-center gap-3 shadow-xl text-left text-center">
        ...
        {/* --- FOOTER FINAL --- */}
        <footer className="bg-black/20 backdrop-blur-3xl border-t border-white/10 py-20 px-6">
        ...
        {!isLogged && <div onClick={() => setShowLogin(true)} className="fixed bottom-0 left-0 w-24 h-24 cursor-default z-[100] opacity-0 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left" title="Admin Login" />}
        ...
        <AnimatePresence>
          {showResenaForm && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-md flex items-center justify-center p-4 text-left">
              <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white dark:bg-[#0a0a0a] p-10 rounded-[3rem] w-full max-w-sm text-center border border-white/10 shadow-2xl text-left text-left">
                <button onClick={() => setShowResenaForm(false)} className="absolute top-8 right-8 text-slate-400 hover:text-rose-500 transition-all text-left text-left text-left"><X /></button>
                <h4 className="text-xl md:text-2xl font-black mb-6 italic uppercase text-indigo-500 text-center text-left text-left">Nueva Reseña</h4>
                <form onSubmit={submitResena} className="space-y-4 text-left">
                  ...
                  <div className="flex flex-col items-center py-2 text-yellow-500 text-left text-left text-left text-left">
                  ...
                  <button type="submit" className="w-full bg-indigo-600 py-4 rounded-xl font-black text-white uppercase tracking-widest text-xs transition-all hover:bg-indigo-700 text-left text-center text-left text-left">Publicar</button>
                </form>
              </motion.div>
            </motion.div>
          )}
          ...
          {showLogin && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[120] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 text-left text-left">
              <div className="bg-white dark:bg-[#0a0a0a] p-10 rounded-[3rem] w-full max-w-sm text-center border border-white/10 shadow-2xl text-left text-left">
                ...
                <button onClick={() => setShowLogin(false)} className="mt-4 text-xs font-bold text-slate-400 uppercase text-left text-left text-left text-left text-left text-left text-left">Cerrar</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {isAdmin ? (
        <div className="space-y-4 text-left">
          ...
          <div className="flex gap-2 text-left">
            ...
            <button onClick={onDelete} className="bg-rose-600 text-white px-4 rounded-2xl transition-all hover:bg-rose-700 text-left text-left text-left text-left text-left text-left text-left text-left"><Trash2 size={16}/></button>
          </div>
        </div>
      ) : (
        <>
